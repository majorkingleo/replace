                ...
                MskTgenericRl *ef_rl_, int reason, void *cbc)
{
	...
	case FCB_LF:
		...
		MskUpdateVar(my_mask, (MskTgeneric *)&ef_StrDrei, KEY_DEF);
		MskUpdateVar(my_mask, (MskTgeneric *)&ef_StrDrei, KEY_10);

		rec->recMngEinh[0] = rec->tst.tstMngEinh;
		MskUpdateVar(my_mask, (MskTgeneric *)&ef_MngEinh2, KEY_DEF);
		...
		if (rec->recWaageIdx != WAAGE_UNDEF) {
			...
			MskVaAssign(my_mask, (MskTgeneric *)&_ef_Waage,
...
static void call_ewd(MskTmaskRlPtr hMaskRl, ZOB_REC *rec)
{
	...
	if (iRv >= 0) {
		if (ewd.RvMng > 0) {
			...
			MskUpdateVar(hMaskRl, (MskTgeneric *)&ef_TeilMng, KEY_DEF);
		}
		/*
		 * NextTab Group auf Menge (wie bei Eingabe)
		 */
		rl = MskQueryRl(hMaskRl, (MskTgeneric *)&ef_TeilMng, KEY_DEF);
		if (rl != NULL)
	...
	case MSK_OK:
		...
		case Step_2:
			...
			if(rv < 0) {
				 ...
				 */
				rl = MskQueryRl(mask_rl, 
					(MskTgeneric *)&ef_dt_def, KEY_DEF);
				if (rl != NULL)
					MskNextTabGroup((MskTgenericRl *)rl);
				MskUpdateMaskVar(mask_rl);
				WdgRemoveAccelerator(mask_rl->shell, &WKAccWaage);
				MskVaAssign(mask_rl, (MskTgeneric *)&_ef_Waage,
					MskNkey,       	(Value) KEY_DEF,
